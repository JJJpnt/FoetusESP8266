...
void artDMXReceived(unsigned char* pbuff) {
  ...
  if ( numberOfDimmers > 0 ) {
    if ( uni == 'A')
      ...
      // long long unsigned int value = static_cast<long long unsigned int>(*pbuff[ARTNET_ADDRESS_OFFSET]);
      Serial.println((int) pbuff[ARTNET_ADDRESS_OFFSET]);
      analogWrite(D3, (int) pbuff[ARTNET_ADDRESS_OFFSET]);   
